...
class TwitterUporabnik():
   ...
   def mnenje(self, tweet):
       """izpise mnenje glede na besedilo twita"""
       cist_twit = textblob.TextBlob(self.ciscenje(tweet)) 
   ...
   def zajemTwitov(self, query, count = 20):
       ...
       twiti = []
       try:
           iskaniTwiti = self.api.search(q = query, count = count)
...
def main(): #glavna funkcija
	...
	print("Positive tweets percentage: {} %".format(100*len(pozTwiti)/len(twiti)))
	print("Negative tweets percentage: {} %".format(100*len(negTwiti)/len(twiti)))
	print("Neutral tweets percentage: {} %".format(100*(len(twiti) - (len(negTwiti) + len(pozTwiti)))/len(twiti)))
